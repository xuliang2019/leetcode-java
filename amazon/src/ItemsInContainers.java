...
//    }
    public int[] itemsContainer(String s, int[] startIndice, int[] endIndice) {

        int n = s.length();
        int total = startIndice.length;
        ...
        for (int j = 0; j < total; j++) {
            int left = startIndice[j];
            int right = endIndice[j];
            ...
            int gap = rightNum - leftNum;
            int cargo = map.get(rightNum) - map.get(leftNum) - gap;
            res[j] = cargo;
